...
import userNotificationsQuery from "./queries/userNotificationsQuery";
 ...
 * @returns
 */
async function fetchAPI(query, { variables } = {}, noCache) {
  // console.log(process.env.NEXT_PUBLIC_STRAPI_API_URL);
  const data = await axios({
    url: `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/graphql${noCache ? "?noCache=true" : ""}`,
    method: "POST",
    headers: {
...
export async function getActiveSponsors(pageSize = 20, offset = 0) {
  try {
...
export async function getUserBySlug(slug) {
  ...
  return data?.usersPermissionsUsers?.data?.length
    ? data?.usersPermissionsUsers?.data[0]
    : null;
}
...
export async function getUserNotifications({ user, pageSize, offset }) {
  const data = await fetchAPIAuthenticated(user, userNotificationsQuery, {
    pageSize,
    offset,
  });
...
export async function getUserNotificationCount({ user, pageSize, offset }) {
  let data = await fetchAPIAuthenticated(user, notificationCountQuery, {
    variables: {
      pageSize,
      offset,
    },
...
export async function getUserRelatedPostsFromId(user, slug) {
  // userArticleQuery
  const data = await fetchAPIAuthenticated(
    user,
    getUserRelatedPostsFromPostId,
    { slug }
  );
  return data;
}
